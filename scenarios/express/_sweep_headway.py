...
this scenario exists for:
    ...
    python scenarios/express/_sweep_headway.py                  3-aspect
    python scenarios/express/_sweep_headway.py etcs_l2
    python scenarios/express/_sweep_headway.py etcs_moving_block
...
Three numbers per row:
  ...
  restrained   seconds the flight spent with its speed held down by a signal
               ...
               itself. Nothing on this railway calls anywhere, so that
               baseline is small - it is the depot roads at each end and
               nothing in between.
...
Both directions run in every sweep. They are independent along the line and,
with nothing calling anywhere, they barely share anything at all - which is
deliberate: this scenario exists to put the binding constraint on the distance
between one train and the next, where a signalling system can reach it.
"""
import os
import sys
...
#:
#: It runs lower than the base scenario's list because this railway holds a
#: great deal more. Nothing calls anywhere, so the cab systems were still
#: all-green at the bottom of that list and it reported the end of the list
#: rather than a limit.
