"""Regenerate scenarios/express/timetable.yaml - the non-stop flight.

Every train on this railway is the same train and none of them call anywhere.
A service is booked at its origin depot and again at its destination depot, and
the three stations in between are run through at whatever the profile allows.

That is the one difference from the base flight, and it is the point of this
scenario: with no platform to queue for, the closest two trains can follow each
other is a property of the signalling and of the line speed, which is the thing
a signalling comparison is trying to measure.
...
possible.
    ...
    python scenarios/express/_generate_timetable.py
    python scenarios/express/_generate_timetable.py 180 timetable-close

Nothing branches off this railway - see the head of infrastructure.yaml. The
line speed is not flat: long runs at 120 with two easings every twelve
kilometres, so the probe runs below are what a train actually achieves over
that profile rather than a flat-out run. Nothing slows for a crossover it is
not taking.
...
BASE = 7 * 3600
DWELL = 45
#: A depot road here is a starting point rather than a turnround: the train is
#: put on it, it goes, and it does not come back within the run. Twenty seconds
#: standing and thirty getting ready keeps the road clear enough that the sweep
#: measures the line rather than the depot - two roads per line at fifty
#: seconds apiece floors the interval around twenty-five, well under anything
#: the signalling reaches.
DEPOT = 20
READY_LEAD = 30
...
COUNT = 8
#: Loose enough that the booked run completes clean; what the line actually
#: holds is what _sweep_headway.py measures.
HEADWAY_S = 240
...
#: (station, dwell) in the order each direction calls at them. Two entries: a
#: service needs an origin and a destination and nothing else, and the stations
#: between them are passed rather than called at. DWELL is therefore unused
#: here, and kept only so the two generators read the same.
UP_PATTERN = [("WDEPOT", DEPOT), ("EDEPOT", DEPOT)]
...
HEADER = '''# express timetable - generated, do not edit by hand.
#
#   python scenarios/express/_generate_timetable.py
#
# %d trains each way, booked %d seconds apart, all of them the same unit, none of
# them calling anywhere. Depot to depot in one run: the three stations in
# between are passed at line speed. Every booked time is what that service
# achieves with the railway to itself.
#
# Same stock and same layout as scenarios/capacity - what differs is that
# nothing stops, and the speed profile it runs over.
